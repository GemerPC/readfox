...
Маршрут `/translate-word` одним запросом получает перевод всего предложения, значение выбранного слова и точный фрагмент этого перевода. Worker проверяет, что фрагмент действительно присутствует в русском предложении, поэтому несогласованные словарные значения не сохраняются.
